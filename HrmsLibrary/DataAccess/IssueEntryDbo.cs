...
namespace HrmsLibrary.DataAccess
{
    ...
    public class IssueEntryDbo
    {
        ...
        public List<IssueEntry> MainGrid(Int32 PageNo, Int32 RowsPerPage, String SearchText, Int64 UserID)
        {
            ...
            {
                con.Open();
                using (cmd = DataFactory.CreateCommand("[Sp_IssueEntryMain]", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public List<IssueEntry> EntryDelete(int EntryId)
        {
            ...
            using (con = DataFactory.CreateConnection())
            {
                con.Open();
                cmd = con.CreateCommand();
                using (cmd = DataFactory.CreateCommand("SP_ISSUEDELETE", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add(DataFactory.CreateParameter("@IssueEntryID",EntryId));
                    RecordAffected = cmd.ExecuteNonQuery();
                }
            }
            if (RecordAffected > 0)
